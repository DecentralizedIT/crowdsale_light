...
var setupCrowdsale = async function(crowdsale, token, whitelist, precision, config, accounts) {
  ...
  await crowdsale.setupMemberAccounts(
    util.config.getWeiValue(config.accounts.lock.stake),
    util.config.getDurationValue(config.accounts.lock.duration))

  // Add trusted nodes
  let shared = MemberAccountShared.at(await crowdsale.memberAccountShared.call())
  await Promise.all(config.accounts.lock.nodes.map(async node => {
      await shared.addNode(
        util.config.getAccountValue(node.account), node.enabled)
  }))

  if (typeof config.authentication.whitelist === 'object') {
    await crowdsale.addAuthenticator(
      typeof config.authentication.whitelist.address === 'string' ? config.authentication.whitelist.address : whitelist.address)    

    if (config.authentication.whitelist.require) {
      await crowdsale.enableAuthentication();
    }
  }
